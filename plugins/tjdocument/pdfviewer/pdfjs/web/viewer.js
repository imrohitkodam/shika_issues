...
function configure(PDFJS) {
  PDFJS.imageResourcesPath = tjPdfViewer.tjDocLibUrl + '/web/images/';
  PDFJS.workerSrc = tjPdfViewer.tjDocLibUrl + '/build/pdf.worker.js';
  PDFJS.cMapUrl = tjPdfViewer.tjDocLibUrl + '/web/cmaps/';
  ...
  load: function load(pdfDocument, scale) {
    ...
    firstPagePromise.then(function (pdfPage) {
      ...
      Promise.all([storePromise, pageModePromise]).then(function (_ref2) {
        ...
        return {
          hash: hash,
          //~ sidebarView: sidebarView // Commented by Irfan
          sidebarView: 0 // Added by Irfan
        };
      }).then(function (_ref4) {
        var hash = _ref4.hash,
...
function getDefaultPreferences() {
  if (!defaultPreferences) {
    defaultPreferences = Promise.resolve({
      "showPreviousViewOnLoad": false, // Changed to False by Irfan
      "defaultZoomValue": "",
      "sidebarViewOnLoad": 0,
...
// Added by Irfan
var DEFAULT_URL = typeof tjPdfViewer.tjDocPdfUrl != "undefined" ? tjPdfViewer.tjDocPdfUrl : 'compressed.tracemonkey-pldi-09.pdf';
;
...
tjPdfViewer.debug = false;
tjPdfViewer.initializeViewer = function(){
	var data = tjPdfViewer.docData;
	var startPage = parseInt(data['current'], 10) ? parseInt(data['current'], 10) : 1;
	var lastSentOn = null;
	var endTime = 0; // End time
	var refreshId = null;
	tjPdfViewer.debug = data['debug'];
	...
	if (data['mode'] != 'preview')
	{
		...
		if (startPage)
		{
			//~ PDFViewerApplication.pdfViewer.currentPageNumber = startPage;
		...
		window.addEventListener('pagechange', function(data){
			sendTrackingRequest();
		});
	}
	else
  ...
  addListenerMulti(window, 'textlayerrendered pagerendered', function(e){
    if (e.detail.pageNumber === PDFViewerApplication.pagesCount) {
      // Finished rendering
      sendTrackingRequest(e.detail.pageNumber);
    }
  });

	function sendTrackingRequest(landScapeLastPage){
		var curTime    = Date.now();
		var total_time = (curTime - lastSentOn) / 1000;
			lastSentOn = curTime;
		var postData = {
				lesson_id: data["lesson_id"],
				current_position: landScapeLastPage ? landScapeLastPage : PDFViewerApplication.page,
				total_time: Math.round(total_time),
				total_content: PDFViewerApplication.pagesCount,
				attempt : data['attempt'],
				mode : ''
			};
		tjPdfViewer.log(postData);
		jQuery.ajax({
				type: "POST",
			...
			.done(function(request) {
				tjPdfViewer.log('done', request);
			})
			.fail(function(request) {
				tjPdfViewer.log('fail', request);
			});
	}
}

tjPdfViewer.log = function(){
	if (tjPdfViewer['debug'])
	{
		console.log('arguments', arguments);
	}
}
window.addEventListener('documentload', function(data){
	hideImage();
	tjPdfViewer.initializeViewer();
});

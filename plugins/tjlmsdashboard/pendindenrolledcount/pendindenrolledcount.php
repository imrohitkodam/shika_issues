...
$lang->load('plg_tjlmsdashboard_pendindenrolledcount', JPATH_ADMINISTRATOR);
...
class PlgTjlmsdashboardPendindenrolledcount extends CMSPlugin
{
	/**
	 ...
	 * @since 1.0.0
	 */
	public function onpendindenrolledcountRenderPluginHTML($plg_data, $layout = 'default')
	{
		$totalpendingEnrollment = $this->getData($plg_data);
	...
	public function getData($plg_data)
	{
		...
		$query->select('COUNT(IF(u.state="0", 1, NULL)) as pending_enrollment');
		...
		$query->where($db->quoteName('u.user_id') . ' = ' . $plg_data->user_id);
		...
		$totalpendingEnrollment = $db->loadresult();

		return $totalpendingEnrollment;
	}
}

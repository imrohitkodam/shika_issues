...
// Check for editing the feedback
...
if (!empty($subformat))
{
	...
	if (!empty($source_option) && $source_plugin == 'feedback')
	{
	...
	<div class="control-group">
		<div class="center">
			<span class="input-append">
				<a onclick="opentjlmsSqueezeBox('index.php?option=com_tmt&view=tests&tmpl=component&addquiz=1&course_id=<?php echo $lesson->course_id; ?>&mod_id=<?php echo $lesson->mod_id; ?>&from_plugin=1&lesson_id=<?php echo $lesson->lesson_id; ?>&qztype=feedback')"
				class="btn btn-info quizaction btn-add-existquiz" title="Add existing feedback">
				<?php echo Text::sprintf('COM_TMT_ADD_EXISTQUIZ',$this->_name);?></a>
			</span>

			<span class="input-append">
				<a class="btn btn-primary btn-create-new-quiz" role="button" onclick="tjfeedback.feedbackSpecificMetadata('<?php echo $form_id; ?>')" ><?php echo Text::_('COM_TMT_CREATE_NEW');?>	</a></span>
			</span>
		</div>
	</div>	<!--control-group-->
</div>	<!--tjquiz_add_quiz-->
...
<div class="quizmetadata_questions">
	<h3 class='center'><?php echo Text::_('PLG_TJFEEDBACK_FEEDBACK_HEADING'); ?></h3>
	...
	<input type="hidden" name="lesson_format[feedback][addq]" id="addq" value = 0 >
	<input type="hidden" name="lesson_format[feedback][test_id]" id="test_id" value="<?php echo $test_id;?>" >
	<input type="hidden" id="subformatoption" name="lesson_format[feedback][subformatoption]" value="test" >
	<input type="hidden" id="qztype" name="lesson_format[feedback][qztype]" value="feedback" >
	<input type="hidden" name="lesson_format[feedback][created_by]" value="<?php echo $user_id; ?>" >
	<input type="hidden" name="lesson_format[feedback][lesson_id]" value="<?php echo $lesson->lesson_id; ?>" >
	<input type="hidden" name="lesson_format[feedback][state]" value="1" >
	<input type="hidden" name="lesson_format[feedback][reviewers][]" value="<?php echo $user_id; ?>" >
...
<script>
	...
	jQuery(document).ready(function(){
		...
		jQuery('#jform_dynamic_question',lesson_format_form).parent().parent().hide();
	});
	...
	function getFormsubFormat(form_id)
	{
		...
		{
			tjfeedback.feedbackSpecificMetadata(form_id);

		}
	}

	function validatefeedbackfeedback()
	{

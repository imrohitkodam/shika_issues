...
class TjlmsTopusersbycoursescompletedDatasource
{
	public $dataSourceName = "PLG_TJDASHBOARDSOURCE_TJLMS_TOP_USERS_COURSES_COMPLETED";
	/**
	...
	public function getData()
	{
		try
		{
			...
			$subquery->select('count(status)');
			$subquery->from($db->qn('#__tjlms_course_track', 't'));
			$condition  = $db->qn('u.id') . '=' . $db->qn('t.user_id');
			$subquery->where($condition . " and status='C'");
			$query->select(array('u.id as id', 'u.name','u.username',"(" . $subquery . ") as count"));
	...
	public function getDataTabulatorTjdashtable()
	{
		...
		$items['columns'] = [
								...
								["title" => 'Course count', "field" => 'count',"formatter" => 'html',"headerSort" => false]
							];

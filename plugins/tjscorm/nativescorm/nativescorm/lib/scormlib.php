...
class tjlmsscormlib
{
	...
	function scorm_parse($scorm, $scormFoldername)
	{
		...
		if (Folder::exists(JPATH_SITE . '/media/com_tjlms/lessons/'.$lesson.'/'. $scormFoldername))
		{
			$manifestfile	=   JPATH_SITE . '/media/com_tjlms/lessons/'.$lesson.'/'. $scormFoldername .'/imsmanifest.xml';
		}
					...
					foreach ($items as $identifier => $item) {
						...
						if (isset($item->sequencingrules)) {
							foreach ($item->sequencingrules as $sequencingrule) {
								...
								$ruleid = $db->insertObject('#__tjlms_scorm_seq_ruleconds', $rule);

...
(function() {
  ...
  jQuery.widget("ui.areacomplete", jQuery.ui.autocomplete, {
    options: jQuery.extend({}, jQuery.ui.autocomplete.prototype.options, {
      matcher: "(\\b[^,]*)",
      suffix: ''
    }),
    _create: function() {
      ...
      this.options.select = jQuery.proxy(this.selectCallback, this);
      this.options.focus = jQuery.proxy(this.focusCallback, this);
    ...
    search: function(value, event) {
      ...
      return jQuery.ui.autocomplete.prototype.search.call(this, this.searchTerm, event);
    },
    _renderItem: function(ul, item) {
      var anchor, li, value;
      li = jQuery('<li>');
      anchor = jQuery('<a>').appendTo(li);
      if (item.image) {
  ...
  jQuery.widget("ui.editablecomplete", jQuery.ui.areacomplete, {
    options: jQuery.extend({}, jQuery.ui.areacomplete.prototype.options, {
      showAtCaret: false
    }),
    selectCallback: function(event, ui) {
      var mention, pos;
  ...
  MentionsBase = (function() {
    ...
    function MentionsBase(input1, options) {
      this.input = input1;
      this.options = jQuery.extend({}, this.settings, options);
  ...
  MentionsInput = (function(superClass) {
    ...
    function MentionsInput(input1, options) {
      ...
      this.highlighterContent = jQuery('div', this.highlighter);
  ...
  MentionsContenteditable = (function(superClass) {
    ...
    mentionTpl = function(mention) {
      return "<strong class='jlike-mentioned-user' data-mention=\"" + mention.uid + "\">" + mention.value + "</strong>";
    };
    ...
    insertMention = function(mention, pos, suffix) {
      ...
      if (suffix) {
        ...
        jQuery(suffix).insertAfter(mention);
    ...
    MentionsContenteditable.prototype._addMention = function(data) {
      ...
      mentionNode = jQuery(mentionTpl(data))[0];
    ...
    MentionsContenteditable.prototype._watch = function(mention) {
      return mention.addEventListener('DOMCharacterDataModified', function(e) {
        ...
        if (e.newValue !== e.prevValue) {
          ...
          jQuery(text).insertBefore(mention);
          jQuery(mention).remove();
    ...
    MentionsContenteditable.prototype.getValue = function() {
      ...
      jQuery(this.selector, value).replaceWith(function() {
        var name, uid;
        uid = jQuery(this).data('mention');
    ...
    MentionsContenteditable.prototype.getMentions = function() {
      var mentions;
      mentions = [];
      jQuery(this.selector, this.input).each(function() {
  ...
  jQuery.fn[namespace] = function() {
    ...
    this.each(function() {
      ...
      if (typeof options === 'string' && options.charAt(0) !== '_') {
        instance = jQuery(this).data('mentionsInput');
        if (options in instance) {
          return returnValue = instance[options].apply(instance, args);
        }
      } else {
        if ((ref = this.tagName) === 'INPUT' || ref === 'TEXTAREA') {
          return jQuery(this).data('mentionsInput', new MentionsInput(jQuery(this), options));
        } else if (this.contentEditable === "true") {
          return jQuery(this).data('mentionsInput', new MentionsContenteditable(jQuery(this), options));
        }
      }
    });
    return returnValue;

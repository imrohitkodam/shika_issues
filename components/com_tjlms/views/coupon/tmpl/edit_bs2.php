...
<script type="text/javascript">
		...
		else
		{
			...
			if (task != 'coupon.cancel' && document.formvalidator.isValid(document.id('coupon-form')))
			{
				...
				if (from_date !== '' &&  exp_date !== '')
				{
					...
					if(from_date > exp_date)
					{
						var dateValidationmsg = Joomla.JText._('COM_TJLMS_COUPON_DATE_VALIDATION');
	...
	function validatecodes(obj,task)
	{
		...
		if (code && code != oldCouponCode)
		{
			jQuery.ajax
			({
				url:"<?php echo JUri::root(true); ?>/index.php?option=com_tjlms&task=coupon.validatecode",
				type: "POST",
...
<div class="<?php echo COM_TJLMS_WRAPPER_DIV; ?>">
	...
	<div class="tjlms_head_title">
		<?php
			...
			?>
		<h2><?php echo (!empty($cop_id))? JText::_("COM_TJLMS_EDIT_COUPON"): JText::_("COM_TJLMS_ADD_COUPON"); ?></h2>
	</div>

	<form action="<?php echo JRoute::_('index.php?option=com_tjlms&view=coupon'. ($cop_id ? ('&id='. $cop_id ) : ''))?>" method="post" enctype="multipart/form-data" name="adminForm" id="coupon-form" class="form-validate form-horizontal ">

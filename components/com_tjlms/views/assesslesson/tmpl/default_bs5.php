...
{
	$userInfo  = Factory::getUser($this->lessonTrack->user_id);
}
	...
	<div class="tjlms-lesson container-fluid mt-45" data-js-attr='tjlms-lesson'>
				...
				<div class="row" data-js-attr="tjlms-lesson__toolbar-container">
					<div class="col-sm-12">
						<span class="ml-10 pull-left font-bold jlike-container">
							<h3 class="mt-10"><?php echo Text::sprintf('COM_TJLMS_HEADING_FOR_ASSESSMENTS', htmlentities($this->lessonAssessment->lesson_title));
							if ($this->lessonAssessment->assessment_student_name)
							{
							?>
								<small>
								<?php
								echo Text::sprintf('COM_TJLMS_ASSESSMENT_SUBMITTED_USER_NAME_', htmlentities($userInfo->name));
								?>
								</small>
								<?php
							}
		...
		<?php if($lesson_data->format != 'quiz') : ?>
			...
			<div id="assessment-form-container" data-js-id="tjlms-sidebar" class="tjlms-lesson__toolbar-content assessment-toggle-main col-sm-4 col-xs-12 p-0 border-0">
				<?php
				$layout  = new FileLayout('assessment.assessment_form');
						...
						{	?>
							<div class="assessment_field form-group">
								<div class="form-inline">
									<button class="btn btn-default submitbtn btn-success"><?php echo Text::_('COM_TJLMS_ASSESSMENTS_SAVE');?></button>
								</div>
								<div class="form-inline">
									<button class="btn btn-default submitbtn btn-success"><?php echo Text::_('COM_TJLMS_ASSESSMENTS_SAVE_N_CLOSE');?></button>
								</div>
							</div>
			<?php 		}
					}
					...
					{	?>
						<div class="assessor-bottom-footer m-0 p-10 text-right">
							...
							<div class="d-inline-block">
								<button type="button" class="btn btn-default" onclick="tjlms.assessment.submit(0);"><?php echo Text::_('COM_TJLMS_ASSESSMENTS_DRAFT');?></button>
							</div>
							<?php endif;?>
							
							<?php if ($this->canEdit):?> 
								<div class="d-inline-block">
									<button type="button"  class="btn btn-default btn-primary" onclick="tjlms.assessment.submit(1);"><?php echo Text::_('COM_TJLMS_ASSESSMENTS_SAVE');?></button>
								</div>
							<?php endif;?>

							<div class="d-inline-block">
								<button type="button" class="btn btn-default btn-danger closeBtn"><?php echo Text::_('COM_TJLMS_CLOSE');?></button>
							</div>
						</div>
				<?php	}	?>
		</div>

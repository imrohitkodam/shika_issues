	...
	public function fetchElement($name, $value, $node, $control_name)
	{
		...
		$jsString =	"<script>
					function checkIfExtInstalled(selectBoxName, extention)
					{
						...
						jQuery('#grpCategoriesField').html('<div class=\'alert alert warning \'>" . Text::_('NO_SOCIAL_GROUPS_CAT_FOUND') . "</div>');
		...
		$options[] = JHTML::_('select.option', 'joomla', Text::_('COM_TJLMS_JOOMLA'));
		$options[] = JHTML::_('select.option', 'jomsocial', Text::_('COM_TJLMS_JOMSOCIAL'));
		$options[] = JHTML::_('select.option', 'easysocial', Text::_('COM_TJLMS_EASYSOCIAL'));

		$fieldName = $name;

		return HTMLHelper::_('select.genericlist',
												$options, $fieldName,
						'class="inputbox tjlmsintegration btn-group form-select" onchange="checkIfExtInstalled(this.name, this.value)" ',

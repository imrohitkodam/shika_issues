...
<form action="<?php echo JRoute::_('index.php?option=com_tjlms&view=userreport'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="table table-striped left_table" id="usersList">
		<thead>
			<tr>
				...
				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_ENROLMENT_USER_ID', 'u.id', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_REPORT_USERNAME', 'u.name', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_REPORT_USERUSERNAME', 'u.username', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_REPORT_USEREMAIL', 'u.email', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_ENROLMENT_TOTAL_COURSES_ENROLLED', 'enrolled_courses', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_ENROLMENT_TOTAL_COURSES_COMPLETED', 'totalCompletedCourses', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_ENROLMENT_TOTAL_COURSES_INCOMPLETED', 'inCompletedCourses', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_ENROLMENT_TOTAL_PENDING_ENROLLED', 'pending_enrollment', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_ENROLMENT_GROUP_TITLE', 'groups', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_ENROLMENT_USER_BLOCKED', 'u.block', $listDirn, $listOrder); ?>
				</th>
		...
		<?php foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					<a class="enrolled_users_cnt" href="<?php echo JUri::root().'administrator/index.php?option=com_tjlms&view=coursereport&filter[userfilter]='.$item->id ?>"  ><?php echo $item->enrolled_courses; ?>
					</a>
				</td>

...
HTMLHelper::_('script', 'administrator/components/com_tjlms/assets/js/ajax_file_upload.js');
HTMLHelper::_('script', 'administrator/components/com_tjlms/assets/js/migration.js');
					...
					<form action="<?php echo Route::_('index.php?option=com_tjlms&view=dashboard');?>" method="post" name="adminForm" id="adminForm" class="col-md-12">
						<div id="filter-bar" class="row date-filter-style">
							<div class="dashboard-filter">
								 <?php if($this->state->get('filter.begin')) {$sdate = $this->state->get('filter.begin');} else {$sdate =  new Date('now -1 month');} ?>
								  <?php if($this->state->get('filter.end')) {$edate = $this->state->get('filter.end');} else {$edate = new Date('now');} ?>
...
function getrecords()
{
	...
	else
	{
		...
		alert('<?php echo $this->escape(Text::_('COM_TJLMS_INVALID_DATE_FIELD')); ?>');
		return false;
	}
}

...
$document->addStyleSheet(Uri::root() . 'components/com_tjlms/assets/css/tjlms.css');
...
function validateSubmit(fromCalendar,elem)
{
	...
	{
		dispEnqueueMessage(Joomla.Text._('COM_TJLMS_MANAGEENROLLMENTS_INVALID_DATE_FORMAT'),'invalid_dates');
	}
	return true;
}
...
function checkValidStartEnd(changedElem)
{
	...
	if(changedElem && sdTimeStamp && edTimeStamp)
	{
		if(sdTimeStamp > edTimeStamp)
		{
			...
			dispEnqueueMessage(Joomla.Text._('COM_TJLMS_MANAGEENROLLMENTS_DATE_RANGE_VALIDATION'),'starts_ends');
			valid = false;
		}
	}
...
{
	if (confirm(Joomla.Text._('COM_TJLMS_CHANGE_COURSE_STATUS')))
	{
...
<?php
	...
	<div class="modal-header">
		...
		<h3><?php echo Text::sprintf('COM_TJLMS_MANAGE_ENROLLED_USERS_FOR_COURSE',$this->courseInfo->title); ?>
	</div>
<?php } ?>
<form action="<?php echo Route::_('index.php?option=com_tjlms&view=manageenrollments'. $courseParam ); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php if (empty($this->items)) : ?>
				<div class="alert alert-no-items">
					<?php echo Text::_('COM_TJLMS_NO_ENROLLMENT_AVAILABLE'); ?>
				</div>
			...
			<table class="table table-striped left_table" id="usersList">
				<thead>
					<tr>
						...
						if (!$courseId)
						{
							if (isset($this->items[0]->id)): ?>
								<th width="1%" class="nowrap center hidden-phone">
									<?php echo JHtml::_('searchtools.sort', 'COM_TJLMS_MANAGEENROLLMENTS_ID', 'b.id', $listDirn, $listOrder); ?>
								</th>
							<?php endif;
						}
						else
						...
						<th class='left'>
							<?php  echo JHTML::tooltip(Text::_('COM_TJLMS_NOTIFY_ASSIGN_USER'), '','', Text::_('COM_TJLMS_NOTIFY_ASSIGN_USER'));?>
						</th>
						...
						<th class='left'>
							<?php  echo JHTML::tooltip(JText::_('COM_TJLMS_SUBSCRIPTION_END_DATE_DESC'), '','', JText::_('COM_TJLMS_SUBSCRIPTION_END_DATE_TITLE'));?>
						</th>
				...
				<?php foreach ($this->items as $i => $item) :
					...
					<td><?php
						...
						?>
						<input type="button" id="assign_<?php echo $i ?>" class="btn btn-success" name="assign" onclick="changeDateCourse(<?php echo $item->course_id ?> , 'notify_user_<?php echo $i ?>', 'start_<?php echo $i ?>', 'due_<?php echo $i ?>', <?php echo $item->todo_id ?>  ,  <?php echo $item->user_id  ?>)"  value="<?php echo Text::_('COM_TJLMS_ASSIGN_USER_BUTTON'); ?>" />
					</td>
					...
					<td>
						...
						<a class="modal studentsReport" href="<?php echo $studenCoursereport; ?>" ><?php echo Text::_('COM_TJLMS_STUDENT_COURSE_REPORT_DASHBOARD'); ?></a>
	...
	<div class="batch_modal">
		<?php if ($canChange)				 : ?>
			<?php echo HTMLHelper::_(
				...
				'collapseModal',
				array(
					'title' => Text::_('COM_TJLMS_USERS_ENROLLMENT_OPTIONS'),

 ...
 * Jlike model.
 */
class JlikeModelRecommendation extends AdminModel
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_jlike.recommendation', 'recommendation', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = Factory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__jlike_todos');
				$max = $db->loadResult();

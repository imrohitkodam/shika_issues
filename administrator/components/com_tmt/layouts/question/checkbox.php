...
?>
<div class="answer-template answer-template-checkbox form-inline row mb-10" id="answer-template-checkbox<?php echo $i;?>">
	<div class="col-lg-3 pl-10">
		...
		<!--
		<input type="text" name="answers_text[]" id="answers_text<?php echo $i;?>" class="inputbox required" size="20" value="<?php echo $answer->answer;?>" placeholder="<?php echo Text::_('COM_TMT_Q_FORM_ANSWER_TEXT_PLACEOLDER');?>"/>
		-->
		<textarea type="text" name="answers_text[]" id="answers_text<?php echo $i;?>" 
		class="inputbox answers_text" rows="5" cols="50"><?php echo $this->escape($answer->answer);?></textarea>
	</div>

	<div class="col-lg-2 answer-media-selection">
			<select data-js-id="answer-media-type" id="answer_media_type<?php echo $i;?>"
				name="answer_media_type[]" class="inputbox" size="1" aria-invalid="false">
				<?php
				foreach ($mediaTypeOptions as $key => $val)
				{
					?>
					<option value="<?php echo $key; ?>" 
					<?php if (isset($mediaData['media_type']) && $key == $mediaData['media_type']) {echo 'selected="selected"';} ?>><?php echo $val; ?></option>
					<?php
				}
				?>
			</select>

			<input class="" data-js-id="answer-media-image" type="file" id="answer_media_image<?php echo $i;?>"
			name="answer_media_image[]" accept="image/*" aria-invalid="false">

			<input class="" data-js-id="answer-media-video" type="text" id="answer_media_video<?php echo $i;?>"
			...
			<input class="" data-js-id="answer-media-file" type="file" id="answer_media_file<?php echo $i;?>"
			...
			if (!empty($mediaData['media_type']))
			{
				...
				<div>
					<?php if ($this->isQuestionAttempted === false) { ?>
						<a class="close" 
						onclick="tmt.tjMedia.deleteMedia('<?php echo $answer->id; ?>', '<?php echo $this->answerMediaClient; ?>', '<?php echo $answer->media_id; ?>');"
						>×</a>
					<?php } ?>
					<?php
	...
	<div class="col-lg-1" data-js-id="mcq-correct">
		<input type="checkbox" name="answers_iscorrect[]" id="answers_iscorrect<?php echo $i;?>" 
		class="answers_iscorrect" value="-1" title="<?php echo Text::_('COM_TMT_Q_FORM_TOOLTIP_ANSWER_MARK_CORRECT');?>" 
		<?php echo $checked;?> <?php echo $readOnly;?> />
	...
	<div class="col-lg-1" data-js-type="quiz">
		...
		class="answers_marks inputbox required validate-whole-number" size="2" value="<?php echo $answer->marks;?>"
		 <?php echo $readOnly;?> />
	</div>

	<div class="col-lg-3">
		<textarea type="text" name="answers_comments[]" id="answers_comments<?php echo $i;?>" 
		class="inputbox" rows="5" cols="50"><?php echo $this->escape($answer->comments);?></textarea>
	</div>

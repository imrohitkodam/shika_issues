 ...
 * @since  1.0.0
 */
class TJMediaStorageLocal implements TjMedia
{
	// Media table id
	public $id = 0;
	...
	public function load($id)
	{
		...
		$mvcFactory = Factory::getApplication()
			...
			->getMVCFactory();
		$table = $mvcFactory->createTable('Files', 'Administrator', ['prefix' => 'TJMediaTable']);

		// Load the object based on the id or throw a warning.
		if (! $table->load($id))
		{
			throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_NO_MEDIA_FILE_IN_MEDIA_TABLE"));
		}
	...
	public function upload($files = array())
	{
		foreach ($files as $file)
		{
			...
			{
				throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_INVALID_FILE_TYPE_ERROR"));
			}
			...
			switch ($fileError)
			{
				case 0:
					break;
				case 4:
					throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_NO_FILE_SENT_ERROR"));
				case 1:
					throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_EXEEDED_FILE_SIZE_LIMIT_ERROR_INI"));
				case 2:
					throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_EXEEDED_FILE_SIZE_LIMIT_ERROR"));
				default:
					throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_UNKOWN"));
			}
			...
			try {
				...
				throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_ERROR_CREATING_FOLDER") . ': ' . $e->getMessage());
			}
			...
			// Upload the image
			try {
				File::upload($fileTmpName, $uploadPath);
			} catch (FilesystemException $e) {
				throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_ERROR_MOVING_FILE") . ': ' . $e->getMessage());
			}
			...
			$this->bind(get_object_vars($this));
			$returnData = array();
			...
			if ($this->saveData)
			{
				...
				$mvcFactory = Factory::getApplication()
					->bootComponent('com_tjlms')
					->getMVCFactory();
				$tjMediaTable = $mvcFactory->createTable('Files', 'Administrator', ['prefix' => 'TJMediaTable']);

				if (!$tjMediaTable->save(get_object_vars($this)))
				{
					throw new \RuntimeException(Text::_("LIB_TECHJOOMLA_MEDIA_ERROR_SAVING_FILE"));
				}
	...
	public function delete()
	{
		...
		$mvcFactory = Factory::getApplication()
			->bootComponent('com_tjlms')
			->getMVCFactory();
		$mediaFilesTable = $mvcFactory->createTable('Files', 'Administrator', ['prefix' => 'TJMediaTable']);

		if (!$mediaFilesTable->delete($this->id))
		{
			throw new \RuntimeException($mediaFilesTable->getError());
		}
		else
		{
			$type = explode('.', $this->type);
